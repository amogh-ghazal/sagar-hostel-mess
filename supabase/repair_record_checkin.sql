-- Repair the check-in RPC used by the Sagar Hostel admin scanner.
-- Safe to run more than once. It does not delete residents or check-in rows.

drop function if exists public.record_checkin(uuid, text);
drop function if exists public.record_checkin(text, uuid);
...
begin
  ...
  if not found then
    raise exception 'This QR code is not registered';
  end if;
  if resident.active is not true then
    raise exception 'Resident access is inactive';
  end if;
  if resident.access_until is not null
     and resident.access_until < current_date then
  ...
  values
    (resident.id, auth.uid(), p_meal_period, 'eligible');
